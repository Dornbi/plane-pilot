...
* **stack headroom** - the unused part of the software stack region. oscar64's
  software stack grows *down* from the end of its region, so the `sections`
...
### 2. Polygon Graphics (7,074 B)

* **Code (5,459 B)**: polygon pipeline, edge scan conversion, near and screen clipping, fixed-point vector math (`poly.cc`, `vec.cc`, `vec_asm.cc`, `fmath.cc`).
...
* **Code (2,824 B)**: viewport split raster handlers, gauge updates, the sprite stack and hardware controller (`view.cc`, `panel.cc`, `sprites.cc`, `spritedef.cc`).
...
* **Code (814 B)**: playback driver, tick handler, note-to-frequency conversion (`music.cc`).
* **Data (1,129 B)**: note table, per-row lead and bass streams, chord table, volume map and mix matrix, bit-packed gate and drum masks.
...
### 11. Core System & Drivers (5,383 B)

* **Code (4,789 B)**: entry point, VIC setup, raster IRQ core, LZO decompressor, keyboard, CPU speed probe, oscar64 runtime (`ppilot.cc`, `mem.cc`, `gfx.cc`, `screen.cc`, `keys.cc`, `cpu.cc`, `bcd.cc`, `print.cc`).
