...
class Model:
    ...
    @property
    def k_max(self) -> int:
        """Largest perspective scale, capped on the buffer WIDTH.

        `2 * max_radius * k / 256 <= MAX_BBOX_W`, solved for k. A constant per
        aircraft type, so the clamp is one comparison and the apparent size
        does not move as the aircraft rotates -- that stability is the whole
        reason the cap is a constant at all.

        Capping on the width rather than on the height gains 15% of silhouette
        for nothing. The price is that the height is then *not* guaranteed: a
        near-knife-edge bank projects the wingspan vertically into 41 rows of
        buffer when the cap allows 46, so the tips clip by a pixel or two. A
        strictly safe cap would have to fit the aircraft inside the largest
        circle the buffer holds -- 41 px of the available 48 -- and give up
        that 15% in every attitude to protect a handful of extreme ones.
        """
        return (MAX_BBOX_W * 128) // self.max_radius
...
def render(state: State, cam: Mat3, target: Mat3, rel_pos_m: Vec3,
    ...
    # 6b. Size clamp. Closer than about 90 m the silhouette outgrows even the
    ...
    # pretending the target stopped approaching. Everything downstream follows
    ...
    # bounding box. Scaling to the box fills the buffer better -- 98% against
    # 87% -- but then the apparent size changes as the aircraft rotates, which
    # reads as the aircraft breathing rather than as a size limit. A constant
    # cap has to fit the aircraft in any orientation at once, so it fits inside
    # the largest circle the buffer holds: 41 px of the available 48. That
    # unused width is the price of a stable size, and it is worth paying.
    ...
    # k is a function of distance alone (the clamp is a constant), so the
    # unforeshortened span in pixels is too, and the fuselage cannot change
    # weight as the aircraft rotates.
