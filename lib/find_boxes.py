...
def generate_boxdefs_c_content(box_defs: Dict[str, Dict[str, Any]],
    ...
    content += "void boxdef_set_main() {\n"
    content += "  if (roll_angle >= kRollMax) {\n"
    content += "    return;\n"
    content += "  }\n"
    content += "  memcpy(&boxdef, main_boxes[roll_angle], sizeof(boxdef_t));\n"
    content += "}\n\n"

    content += "void boxdef_set_alt() {\n"
    content += "  if (roll_angle >= kRollMax) {\n"
    content += "    return;\n"
    content += "  }\n"
    content += "  if (alt_boxes[roll_angle] == NULL) {\n"
    content += "    boxdef_set_main();\n"
    content += "  } else {\n"
    content += "    memcpy(&boxdef, alt_boxes[roll_angle], sizeof(boxdef_t));\n"
    content += "  }\n"
    content += "}\n\n"
