...
- **Per-Stage Cycle Counters for Polygons, Clouds and Sprites**: The debug view (`D`) now reports `PLY`, `CLD` and `SPR` — the cost of polygon drawing, the cloud scan and the sprite stack — as a breakdown of the existing `GRD` and `UPD` stages. Parked on the runway, the single runway polygon is 43,128 cycles: 53% of the grid walk and a third of the whole measured frame.
