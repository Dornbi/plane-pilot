#!/usr/bin/env python3
"""
Analyzes an oscar64 .map file and reports RAM usage broken down by feature.

Categories memory into Code, Static Data/LUTs, Dynamic BSS/ZeroPage, and Video RAM.
...
def parse_map(map_path):
    ...
    categories = {
        'Horizon Graphics': {'Code': 0, 'StaticData': 0, 'DynamicRAM': 0, 'VideoRAM': 0, 'items': []},
        'Polygon Graphics': {'Code': 0, 'StaticData': 0, 'DynamicRAM': 0, 'VideoRAM': 0, 'items': []},
        'World Model': {'Code': 0, 'StaticData': 0, 'DynamicRAM': 0, 'VideoRAM': 0, 'items': []},
        'Instrument Panel': {'Code': 0, 'StaticData': 0, 'DynamicRAM': 0, 'VideoRAM': 0, 'items': []},
        'Menu & Missions': {'Code': 0, 'StaticData': 0, 'DynamicRAM': 0, 'VideoRAM': 0, 'items': []},
        'Message System': {'Code': 0, 'StaticData': 0, 'DynamicRAM': 0, 'VideoRAM': 0, 'items': []},
        'Sound': {'Code': 0, 'StaticData': 0, 'DynamicRAM': 0, 'VideoRAM': 0, 'items': []},
        'Core System & Drivers': {'Code': 0, 'StaticData': 0, 'DynamicRAM': 0, 'VideoRAM': 0, 'items': []},
    }
    ...
    for o in objects:
        ...
        kind = 'StaticData'
        if 'code' in sec.lower() or 'startup' in sec.lower():
            kind = 'Code'
        elif 'zeropage' in sec.lower() or 'bss' in sec.lower() or 'stack' in sec.lower():
            kind = 'DynamicRAM'
        elif 'data' in sec.lower():
            kind = 'StaticData'
        
        categories[cat][kind] += sz
        categories[cat]['items'].append((o['name'], sz, sec, kind))
    ...
    categories['Horizon Graphics']['VideoRAM'] += 1792 + 1000 + 1000
    categories['Instrument Panel']['VideoRAM'] += 256
    categories['Core System & Drivers']['VideoRAM'] += 1000

    return categories


def main():
    parser = argparse.ArgumentParser(description='Analyze Plane Pilot RAM map')
    ...
    if args.markdown:
        print(f"| Feature Area | Code (CPU Inst.) | Static Data / LUTs | Dynamic BSS / ZeroPage | Video RAM / Hardware Buffers | **Total Footprint** |")
        print(f"| :--- | :---: | :---: | :---: | :---: | :---: |")
        ...
        for cname, cat in categories.items():
            ...
            kb = tot / 1024.0
            print(f"| **{idx}. {cname}** | {code:,} B | {data:,} B | {dyn:,} B | {vram:,} B | **{tot:,} B ({kb:.1f} KB)** |")
            idx += 1

        print(f"| **TOTAL** | **{total_code:,} B** | **{total_data:,} B** | **{total_dyn:,} B** | **{total_vram:,} B** | **{total_all:,} B ({total_all/1024.0:.1f} KB)** |")

    else:
        print(f"\nRAM Analysis for: {args.map_file}\n")
        print(f"{'Category':<25} | {'Code (B)':<10} | {'Data/LUT (B)':<12} | {'Dyn RAM (B)':<12} | {'Video RAM (B)':<13} | {'Total (B)':<10}")
        print("-" * 95)
        ...
        for cname, cat in categories.items():
            ...
            print(f"{cname:<25} | {code:<10} | {data:<12} | {dyn:<12} | {vram:<13} | {tot:<10}")

        print("-" * 95)
        print(f"{'TOTAL':<25} | {total_code:<10} | {total_data:<12} | {total_dyn:<12} | {total_vram:<13} | {total_all:<10}")
