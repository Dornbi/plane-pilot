...
# Compass letters, drawn in the black surround outside the map. Stencils like
# the digits: the surround is solid 11 over black color RAM, so punching 01
# into it gives white letters for free. Four multicolor pixels is narrow for
# letterforms -- N and W use all four columns and are still tight -- but the
# position around the map carries most of the meaning anyway.
COMPASS_NAMES = ["COMPASS_N", "COMPASS_E", "COMPASS_S", "COMPASS_W"]
STENCIL_NAMES = DIGIT_NAMES + COMPASS_NAMES
...
def _read_sheet(path):
    ...
    expected = len(TILE_NAMES) + len(STENCIL_NAMES)
...
def _encode_stencil(rows, label):
    """Overlay stencil -> 8 mask bytes with 11 in each ink pair."""
    masks = []
    for y, row in enumerate(rows):
        mask = 0
        for x, color in enumerate(row):
            if color == OVERLAY:
                ...
                sys.exit(
                    ...
                    f"{color}; overlay stencils may only use white (ink) "
...
def _write_header(path, tiles, digits, compass):
    with open(path, "w") as f:
        f.write(
            ...
            f"static const uint8_t kMapCompassCount = {len(compass)};\n"
            "\n"
        ...
        for i, name in enumerate(COMPASS_NAMES):
            f.write(f"static const uint8_t kMapCompass{name[-1]} = {i};\n")
        f.write(
            "\n"
...
def _write_source(path, tiles, digits, compass):
    ...
    with open(path, "w") as f:
        ...
        f.write("const uint8_t kMapCompassMask[kMapCompassCount][8] = {\n")
        for name, mask in zip(COMPASS_NAMES, compass):
            f.write(f"    {{{_hex_row(mask)}}}, // {name}\n")
        f.write("};\n// clang-format on\n")
...
def main():
    ...
    stencils = [
        _encode_stencil(raw[len(TILE_NAMES) + i], STENCIL_NAMES[i])
        for i in range(len(STENCIL_NAMES))
    ]
    digits = stencils[: len(DIGIT_NAMES)]
    compass = stencils[len(DIGIT_NAMES):]

    _write_header(args.out_h, art, digits, compass)
    _write_source(args.out_cc, art, digits, compass)

    if not args.quiet:
        total = len(art) * 10 + len(stencils) * 8
        print(f"generate_map_tiles: {len(art)} tiles, {len(digits)} digits, "
              f"{len(compass)} compass letters")
